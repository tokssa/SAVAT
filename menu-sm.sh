...
egrep "^open_scrip" /root/.profile >/dev/null
if [ $? -eq 0 ]; then
clear
else
echo "open_scrip" > /root/.profile
fi

sc_rip=https://smile-vpn.net/sc_rip
scrip=https://smile-vpn.net/scrip

if [[ -e /home/vps/public_html/file ]]; then
echo
else
...
if [[ -e /home/vps/public_html/file/index.php ]]; then
echo
else
...
wget -q -O panel $sc_rip/sm_sc/panel/sm-panel.sh > /dev/null 2>&1
bash panel
exit 0
else
clear
...
up )
cd /usr/local/bin
wget $sc_rip/sm_sc/openvpn/menu_sm/menu-sm.sh > /dev/null 2>&1
